...
function EmberApp(options) {
  ...
  sandbox.run(vendorFile, vendorFilePath).default;
  debug("vendor file evaluated");

  sandbox.run(appFile, appFilePath).default;
  debug("app file evaluated");

  let config = sandbox.require(appName+'/config/environment').default;
  let app = sandbox.require(appName+'/app').default;

  this._app = app.create(config.APP);
}
...
EmberApp.prototype.visit = function(url) {
  ...
  var options = { isBrowser: false, document: doc, rootElement: rootElement, autoRun: false };
